# Confeitaria distribuída com Redis Streams

Repositório da atividade: [Renovato86/confeitaria_Sistemas_Distribu-dos](https://github.com/Renovato86/confeitaria_Sistemas_Distribu-dos).

Para começar, baixe os arquivos em **Code > Download ZIP**, extraia o ZIP e siga a seção **Começar no Windows** abaixo. O [relatório em texto](docs/relatorio.md), o [relatório editável](docs/Relatorio_Confeitaria_Redis_Streams.docx) e o [roteiro de apresentação](docs/roteiro-apresentacao.md) estão na pasta `docs`.

Atividade prática de Sistemas Distribuídos: um balcão cadastra pedidos e trabalhadores da cozinha processam as fichas. O Redis recebe as mensagens e intermedeia toda a comunicação. Não existe chamada direta do balcão para os trabalhadores.

O programa tem catálogo, validação das quantidades, cálculo do total, registro do resultado e consulta dos pedidos. Os preços e os clientes são fictícios. O tempo de preparação é simulado; o resultado real do processamento é uma ficha digital.
...
Extraia o ZIP. Abra o PowerShell **dentro da pasta que contém este README e o arquivo `compose.yaml`**.
